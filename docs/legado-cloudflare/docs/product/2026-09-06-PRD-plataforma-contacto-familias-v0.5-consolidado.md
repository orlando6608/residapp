...
**Estado:** Consolidado funcional aprobado para la línea base incremental v1.1  
...
**Documento sustituido:** PRD v0.4, de 5 de septiembre de 2026  
**Documento asociado vigente:** Matriz de permisos de seis perfiles v0.2.1  
...
2. Este PRD v0.5 para alcance, invariantes, entidades y comportamiento global.
...
> **Decisión incremental de la v0.5.** Se cierran `D1-P01` y `D1-P08`: catálogo administrativo de sexo documentado compatible con FHIR, distinción entre `NINGUNA` y `NO_DOCUMENTADO` en ayuda técnica, aplicabilidad de textura/líquidos en alimentación enteral y descripción obligatoria de opciones `OTRO/OTRA`. No se amplían permisos y `D1-P04`–`D1-P06` continúan diferidas.
...
- **RES-01 P0.** Ficha con nombre, fecha de nacimiento/edad calculada, sexo documentado, unidad y ubicación aplicable. Sexo documentado utiliza el catálogo cerrado FHIR `male`, `female`, `other` y `unknown`, mostrado como Hombre, Mujer, Otra categoría documentada y No consta; procede de una fuente administrativa y nunca se infiere ni solicita texto libre. Los identificadores son opacos y no secuenciales y no se usan como autorización.
...
- **BAS-16 P0.** Movilidad separa modo habitual, ayuda técnica, ayuda humana y transferencias. En ayuda técnica, `NINGUNA` significa ausencia conocida, `NO_DOCUMENTADO` significa información desconocida y `NO_APLICA` no está permitido; `OTRA` exige descripción breve. Las ayudas técnicas de movilidad no se duplican en Ayudas habituales.
- **BAS-17 P0.** Alimentación distingue al menos textura normal, troceada, triturada y puré; líquidos sin espesante, néctar, miel, pudín o no documentado. `NO_APLICA` solo se admite para textura y consistencia de líquidos con vía exclusivamente `ENTERAL`; una vía `MIXTA` exige registrar la parte oral o `NO_DOCUMENTADO`.
- **BAS-18 P0.** Fuente y fecha pertenecen a la versión basal común. Las opciones incompatibles se modelan como excluyentes. Toda opción estructurada `OTRO` u `OTRA`, incluidas `OTRA_TEXTURA_ADAPTADA` y `OTRO_PRODUCTO_DE_APOYO`, exige una descripción breve no vacía; sin la opción correspondiente, el texto permanece vacío.
...
| --- | --- | --- |
| 1. Consolidación documental | Decisiones aprobadas + PRD v0.5 + matriz v0.2.1 + wireframes vigentes + trazabilidad v0.2 | Ninguno |
...
# Anexo B. Cambios de v0.4 a v0.5

- Se cierra `D1-P01` con los códigos `male`, `female`, `other` y `unknown`, etiquetas localizadas y prohibición de inferencia o texto libre.
- Se cierra `D1-P08`: `NINGUNA` sustituye a `NO_APLICA` en ayuda técnica; `NO_APLICA` se restringe a textura/líquidos de vía exclusivamente enteral; las opciones `OTRO/OTRA` exigen descripción.
- Se mantienen los 140 identificadores funcionales existentes; cambian catálogos y criterios de aceptación, no el número de requisitos.
- No se modifican permisos. `D1-P04`, `D1-P05` y `D1-P06` permanecen diferidas y denegadas por defecto.
- Esta versión sustituye a v0.4 como referencia funcional corriente, sin alterar la evidencia histórica de la línea base v1.
